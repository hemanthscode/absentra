...
exports.validateAgainstPolicy = async (leaveType, startDate, endDate, employee) => {
  ...
  if (days > policy.maxConsecutiveDays) {
    return {
      isValid: false,
      message: `Cannot apply for more than ${policy.maxConsecutiveDays} consecutive ${leaveType} leaves`
    };
  }
  
  // Check department applicability
  if (!policy.isDepartmentApplicable(employee.department)) {
    return {
      isValid: false,
  ...
  if (!policy.isDesignationApplicable(employee.designation)) {
    return {
      isValid: false,
...
exports.calculateRemainingLeave = (employee, leaveType, year) => {
  // This would need to consider leaves taken in the year
